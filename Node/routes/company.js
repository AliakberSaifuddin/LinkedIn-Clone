...
router.post('/registerCompany', function (req, res, next) {
    console.log(req.body.data);
    var c_data = req.body.data;
    console.log('company', c_data);
    // var query = 
    // console.log(query);
    connection.query('insert into company (name, email, password, address, phoneNumber, website) values (?,?,?,?,?,?)',
    [c_data.name, c_data.email, c_data.password, c_data.address, c_data.phoneNumber, c_data.website], function (error, result) {
        console.log('rows', result);
        if (result) {
            console.log('here')
            res.send(JSON.stringify(result))
